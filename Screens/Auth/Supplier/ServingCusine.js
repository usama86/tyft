...
import { StyleSheet, TouchableOpacity, View, Image, FlatList, ActivityIndicator } from 'react-native';
...
const ServingCusine = ({ navigation, route }) => {
	...
	const PrintCard = (item, index) => (
		<TouchableOpacity
			...
			activeOpacity={0.8}
			style={[ styles.MainView, item.checked ? { backgroundColor: theme.colors.primary, borderWidth: 0 } : null ]}
		>
			<Text style={[ item.checked ? { color: 'white' } : null ]} value={item.cusineName} />
		</TouchableOpacity>
	);
